...
orderExpress.post('/submitOrder', async (req, res, next) => {
    const sql = 'CALL sp_create_order(:userid, :user_address_id, :order_id_status, :razor_payment_id,:orderid)';
    ...
    try {
        db.doConnect(async (err, connection) => {
            try {
                ...
                if (result != null) {
                    res.status(200).send({ message: 'Order has been submitted', isSuccess: true, OrderId: result.outBinds.orderid });
                }
                else {
                    res.status(200).send({ message: 'Unable to take order', isSuccess: false });
                }
            } catch (err) {
